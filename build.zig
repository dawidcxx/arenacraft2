...
/// Scans `src/game_data/db/*.json` at configure time and emits a generated
/// `game_data_db` module. Each `foo.json` (a JSON array of homogeneous
/// objects) becomes a generated `foo.zig` exposing `Row` (the inferred
/// element struct) and `rows: []const Row`, re-exported as
/// `game_data_db.foo`. `build()` re-runs on every `zig build`, so edits to
/// the JSON files are picked up automatically.
fn generateGameDataDbModule(b: *std.Build) !*std.Build.Module {
    ...
    try root_src.appendSlice(gpa, "// GENERATED by build.zig from src/game_data/db/*.json - DO NOT EDIT.\n");
...
fn generateGameDataFile(b: *std.Build, json_bytes: []const u8) ![]const u8 {
    ...
    try src.appendSlice(gpa, "// GENERATED from a src/game_data/db/*.json file by build.zig - DO NOT EDIT.\n");
