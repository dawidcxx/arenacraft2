...
```
game_data/db/spells.json ────────┐
game_data/db/skills.json ────────┤
game_data/db/initial_spells.json ┤ build.zig codegen (re-runs on every build)
game_data/db/initial_skills.json ┘
                                 v
...
- `spells.json` mirrors the 3.3.5a Spell.dbc subset we implement (entry,
  school, cast time, damage range, aura bits). Duplicate entries are a
  compile error, same as items.
- `initial_spells.json` / `initial_skills.json` grants:
...
as `skills.json` (skill_id → spell_id); grants send both sides from one
row.
