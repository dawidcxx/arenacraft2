...
//!
//! Wire data only: marshal/unmarshal plus the byte constants the packets
//! share. Cast gating, interruption rules and damage application live in
//! the world/server layers.
//!
//! Layouts transcribed from the reference core: Spell.cpp (SendSpellStart /
...
/// SpellCastTargetFlags: the unit-target bit.
pub const target_flag_unit: u32 = 0x0000_0002;

/// Target masks whose bit is followed by a packed guid on the wire:
/// UNIT, CORPSE_ENEMY, GAMEOBJECT, CORPSE_ALLY, UNIT_MINIPET.
const target_flags_with_guid: u32 =
    target_flag_unit | 0x0000_0200 | 0x0000_0800 | 0x0000_8000 | 0x0001_0000;

/// CastFlags written by SMSG_SPELL_START: trajectory data follows.
pub const cast_flag_has_trajectory: u32 = 0x0000_0002;
/// CastFlags written by SMSG_SPELL_GO; retail always sets it.
pub const cast_flag_unknown_9: u32 = 0x0000_0100;
/// CastFlags on CMSG_CAST_SPELL: projectile elevation/speed follow the
/// target block. Unread for now; the server decides trajectories.
...
/// AuraFlags for SMSG_AURA_UPDATE (AFLAG_*). The caster guid is appended
/// unless aura_flag_caster is set; durations unless aura_flag_duration is
/// clear.
pub const aura_flag_eff_index_0: u8 = 0x01;
pub const aura_flag_caster: u8 = 0x08;
...
/// CMSG_CAST_SPELL (0x12E): u8 cast count, u32 spell id, u8 cast flags,
/// then the SpellCastTargets block (u32 target mask + packed guid for
/// guid-carrying masks). Everything after the target guid — projectile
/// elevation/speed, movement data, coordinates, string targets — stays
/// unread.
pub const CastSpellClient = struct {
    cast_count: u8,
    spell_id: u32,
    cast_flags: u8,
    /// Object target guid; null when the mask carries no guid (self casts,
    /// coordinate or string targets).
    target_guid: ?ObjectGuid,

    pub fn unmarshal(bytes: []const u8) ProtocolError!CastSpellClient {
        var cursor = Cursor{ .bytes = bytes };

        const cast_count = cursor.readU8() orelse return ProtocolError.InvalidMessage;
        const spell_id = cursor.readU32() orelse return ProtocolError.InvalidMessage;
        const cast_flags = cursor.readU8() orelse return ProtocolError.InvalidMessage;
        const target_mask = cursor.readU32() orelse return ProtocolError.InvalidMessage;

        var target_guid: ?ObjectGuid = null;
        if (target_mask & target_flags_with_guid != 0) {
            const read = ObjectGuid.fromPacked(cursor.remaining()) catch return ProtocolError.InvalidMessage;
            target_guid = read.guid;
        }
...
/// packs guids inside SpellCastTargets; simple combat/query packets carry
...
/// CMSG_SET_SHEATHED (0x1E0): u32 SheathState (0 unarmed, 1 melee, 2
/// ranged). The reference core stores it in UNIT_FIELD_BYTES_2 byte 0,
/// which propagates the pose to other clients.
...
/// SMSG_SPELL_START (0x131): a cast began; the client shows the cast bar
...
pub const SpellStartServer = struct {
    ...
    pub fn marshal(self: SpellStartServer, gpa: std.mem.Allocator) ![]u8 {
        ...
        try appendUnitTargets(&out, gpa, self.target_guid);
...
/// SMSG_CAST_FAILED (0x130): a cast was rejected; the client shows the red
/// error text.
...
/// when a running cast is interrupted. Both carry the same body in the
/// reference core; failure additionally shows the caster an error text.
...
/// SMSG_ATTACKSTOP (0x144): packed guids + u32 dead flag. A null victim
/// omits both the guid and the flag.
...
/// Targets.Write for a unit-targeted spell: u32 mask + packed guid, or a
/// zero mask when there is no target.
fn appendUnitTargets(out: *std.ArrayList(u8), gpa: std.mem.Allocator, target_guid: ?ObjectGuid) !void {
    if (target_guid) |target| {
        ...
        try appendPackedGuid(out, gpa, target);
    } else {
        try appendU32(out, gpa, 0);
    }
}
...
fn appendPackedGuid(out: *std.ArrayList(u8), gpa: std.mem.Allocator, guid: ObjectGuid) !void {
    try out.appendSlice(gpa, guid.toPacked().slice());
}
...
/// Bounds-checked little-endian reader for client packets.
const Cursor = struct {
    bytes: []const u8,
    offset: usize = 0,

    fn remaining(self: *const Cursor) []const u8 {
    ...
    fn readU32(self: *Cursor) ?u32 {
        if (self.bytes.len - self.offset < 4) return null;
...
test "cast spell client reads unit target and skips trajectory bytes" {
    ...
    try body.append(t.allocator, client_cast_flag_projectile);
    ...
    try t.expectEqual(client_cast_flag_projectile, parsed.cast_flags);
    try t.expectEqual(caster.valueOf(), parsed.target_guid.?.valueOf());
}

test "cast spell client reads the guid behind every guid-carrying mask" {
    const t = std.testing;

    const target = ObjectGuid.player(9);
    const guid_bytes = target.toPacked().slice();

    // UNIT, CORPSE_ENEMY, GAMEOBJECT, CORPSE_ALLY, UNIT_MINIPET.
    for ([_]u32{ 0x2, 0x200, 0x800, 0x8000, 0x10000 }) |mask| {
        var buf: [12]u8 = .{0} ** 12; // header + mask + 2-byte packed guid
        std.mem.writeInt(u32, buf[1..5], 1, .little);
        std.mem.writeInt(u32, buf[6..10], mask, .little);
        @memcpy(buf[10..], guid_bytes);

        const parsed = try CastSpellClient.unmarshal(&buf);
        try t.expectEqual(target.valueOf(), parsed.target_guid.?.valueOf());
    }
}

test "cast spell client rejects truncated payloads" {
    const t = std.testing;

    try t.expectError(ProtocolError.InvalidMessage, CastSpellClient.unmarshal(&.{}));
    try t.expectError(ProtocolError.InvalidMessage, CastSpellClient.unmarshal(&.{ 1, 0x74, 0, 0 }));
    try t.expectError(ProtocolError.InvalidMessage, CastSpellClient.unmarshal(&.{ 1, 0x74, 0, 0, 0, 2, 0, 0, 0 }));
    // Mask says a guid follows but the guid is cut short.
    try t.expectError(ProtocolError.InvalidMessage, CastSpellClient.unmarshal(&.{ 1, 0x74, 0, 0, 0, 2, 0, 0, 0x03, 0x34 }));
}
...
test "aura update omits the caster guid under the caster flag" {
    const t = std.testing;
    const gpa = t.allocator;
    ...
    const body = try (AuraUpdateServer{
        ...
        .slot = 1,
        .spell_id = 116,
        .flags = aura_flag_eff_index_0 | aura_flag_caster,
    }).marshal(gpa);
    defer gpa.free(body);
