...
pub const UnitField = enum(u16) {
    ...
    /// UNIT_MOD_CAST_SPEED (float). Retail sets 1.0 at player create
    /// ("fix cast time showed in spell tooltip on client"); the client
    /// derives a watched unit's cast animation timing from it — a zero
    /// here leaves other clients showing the spell kit without the cast
    /// animation.
    mod_cast_speed = 80,
    bytes_2 = 122,
    // Non-exhaustive: the client accepts any index in range.
