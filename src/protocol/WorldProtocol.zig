...
test AuthChallengeServer {
    const t = std.testing;
    const pkt = AuthChallengeServer{
        .seed = [_]u8{ 0xAA, 0xBB, 0xCC, 0xDD },
        .seeds = [_]u8{0x11} ** 32,
    };
    const wire = try pkt.marshal(t.allocator);
    defer t.allocator.free(wire);
    try t.expectEqual(@as(usize, 40), wire.len);
    try t.expectEqual(@as(u32, 1), std.mem.readInt(u32, wire[0..4], .little));
    try t.expectEqual(@as(u8, 0xAA), wire[4]);
    try t.expectEqual(@as(u8, 0xDD), wire[7]);
    try t.expectEqual(@as(u8, 0x11), wire[8]);
    try t.expectEqual(@as(u8, 0x11), wire[39]);
}

test AuthSessionClient {
    const t = std.testing;
    const bytes = [_]u8{
        0x34, 0x30, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00,
    ...
    const parsed = try AuthSessionClient.unmarshal(&bytes);
    try t.expectEqual(@as(u32, 12340), parsed.build);
    try t.expectEqualStrings("admin", parsed.account);
    try t.expectEqual(@as(u32, 1), parsed.realm_id);
    try t.expectEqual(@as(u8, 0xCC), parsed.digest[0]);
    try t.expectEqual(@as(u8, 0xCC), parsed.digest[19]);
    try t.expectEqual(@as(usize, 0), parsed.addon_info.len);
}

test AuthResponseServer {
    const t = std.testing;
    {
        const wire = try (AuthResponseServer.unknownAccount()).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqual(@as(usize, 1), wire.len);
        try t.expectEqual(@as(u8, 0x15), wire[0]);
    }
    {
        const wire = try (AuthResponseServer.success(.wotlk)).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqual(@as(usize, 11), wire.len);
        try t.expectEqual(@as(u8, 0x0C), wire[0]);
        try t.expectEqual(@as(u8, 2), wire[10]);
    }
}
...
test "world frame headers" {
    const t = std.testing;

    {
        const encoded = try (Frame.Header{
            .opcode = .smsg_auth_challenge,
            .payload_len = AuthChallengeServer.total_size,
        }).encodeServer();
        try t.expectEqual(@as(usize, 4), encoded.len);
        try t.expectEqualSlices(u8, &.{ 0x00, 0x2A, 0xEC, 0x01 }, encoded.slice());
    }

    {
        var client_header = [_]u8{ 0x00, 0x08, 0xED, 0x01, 0x00, 0x00 };
        const client = try Frame.Header.fromClientBuf(&client_header);
        try t.expectEqual(Opcode.cmsg_auth_session, client.opcode);
        try t.expectEqual(@as(usize, 4), client.payload_len);
    }

    {
        ...
        try t.expectEqual(@as(u8, 0x80), encoded.bytes[0]);
        try t.expectEqual(@as(u8, 0x80), encoded.bytes[1]);
        try t.expectEqual(@as(u8, 0x02), encoded.bytes[2]);
    }
}

test "small world packet bodies" {
    const t = std.testing;

    {
        const wire = try (PongServer{ .ping = 0xAABBCCDD }).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqualSlices(u8, &.{ 0xDD, 0xCC, 0xBB, 0xAA }, wire);
    }
    {
        const wire = try (AccountDataTimesServer{ .unix_time = 123, .mask = AccountDataTimesServer.global_cache_mask }).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqual(@as(usize, 21), wire.len);
        try t.expectEqual(@as(u32, AccountDataTimesServer.global_cache_mask), std.mem.readInt(u32, wire[5..9], .little));
    }
    {
        const wire = try (AddonInfoServer{ .addons = &.{} }).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqual(@as(usize, 4), wire.len);
        try t.expectEqual(@as(u32, 0), std.mem.readInt(u32, wire[0..4], .little));
    }
    {
        const wire = try (CharEnumServer{ .characters = &.{} }).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqualSlices(u8, &.{0}, wire);
    }
    {
        var chars = [_]CharEnumEntry{.{
            .guid = ObjectGuid.player(7),
            .name = "Bob",
            .race = 1,
            .class = 1,
            .gender = 0,
        ...
        chars[0].equipment[4] = .{ .display_id = 13116, .inventory_type = 5, .enchant_aura = 0 };
        const wire = try (CharEnumServer{ .characters = &chars }).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqual(@as(u8, 1), wire[0]);
        try t.expectEqual(@as(u64, 7), std.mem.readInt(u64, wire[1..9], .little));
        try t.expectEqualStrings("Bob", wire[9..12]);
        try t.expectEqual(@as(u32, 12), std.mem.readInt(u32, wire[22..26], .little));

        // Equipment block follows pet info. For name "Bob" it begins at offset 67;
        // slot 4 (chest) starts at 67 + 4*9 = 103.
        const chest_off: usize = 103;
        try t.expectEqual(@as(u32, 13116), std.mem.readInt(u32, wire[chest_off..][0..4], .little));
        try t.expectEqual(@as(u8, 5), wire[chest_off + 4]);
        try t.expectEqual(@as(u32, 0), std.mem.readInt(u32, wire[chest_off + 5 ..][0..4], .little));
    }
    {
        const parsed = try CharCreateClient.unmarshal("Bob\x00\x01\x02\x00\x03\x04\x05\x06\x07\x08");
        try t.expectEqualStrings("Bob", parsed.name);
        try t.expectEqual(@as(u8, 1), parsed.race);
        try t.expectEqual(@as(u8, 8), parsed.outfit_id);
    }
    {
        const parsed = try CharDeleteClient.unmarshal(&.{ 0x07, 0, 0, 0, 0, 0, 0, 0 });
        try t.expectEqual(@as(u64, 7), parsed.guid.valueOf());
        try t.expectError(ProtocolError.InvalidMessage, CharDeleteClient.unmarshal(&.{ 0x07, 0, 0 }));

        const wire = try (CharDeleteServer{ .code = CharDeleteServer.success }).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqualSlices(u8, &.{0x47}, wire);
    }
}

test "player login client" {
    const t = std.testing;
    const parsed = try PlayerLoginClient.unmarshal(&.{ 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 });
    try t.expectEqual(@as(u64, 7), parsed.guid.valueOf());
    try t.expectError(ProtocolError.InvalidMessage, PlayerLoginClient.unmarshal(&.{ 0x07, 0x00, 0x00 }));
}

test "login verify world" {
    const t = std.testing;
    const pkt = LoginVerifyWorldServer{
        .map_id = 1,
        .position_x = 100.0,
        .position_y = 200.0,
        .position_z = 300.0,
    ...
    const wire = try pkt.marshal(t.allocator);
    defer t.allocator.free(wire);
    try t.expectEqual(@as(usize, 20), wire.len);
    try t.expectEqual(@as(u32, 1), std.mem.readInt(u32, wire[0..4], .little));
}

test "login burst packets marshal to exact wire shapes" {
    const t = std.testing;

    {
        const wire = try (BindPointUpdateServer{ .x = 1.5, .y = 2.5, .z = 3.5, .map_id = 0, .area_id = 12 }).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqual(@as(usize, 20), wire.len);
        try t.expectEqual(@as(u32, 12), std.mem.readInt(u32, wire[16..20], .little));
    }

    {
        const wire = try (TalentsInfoServer{}).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqual(@as(usize, TalentsInfoServer.body_len), wire.len);
        try t.expectEqual(@as(u8, 0), wire[0]); // player, not pet
        try t.expectEqual(@as(u8, 1), wire[5]); // one spec group
        try t.expectEqual(@as(u8, 6), wire[8]); // glyph slots
    }

    {
        const wire = try (InitialSpellsServer{}).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqualSlices(u8, &.{ 0, 0, 0, 0, 0 }, wire);
    }

    {
        const wire = try (InitialSpellsServer{ .spells = &.{668} }).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqualSlices(u8, &.{ 0, 1, 0, 0x9C, 0x02, 0, 0, 0, 0, 0, 0 }, wire);
    }

    {
        const wire = try (LearnedSpellServer{ .spell_id = 668 }).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqualSlices(u8, &.{ 0x9C, 0x02, 0, 0, 0, 0 }, wire);
    }

    {
        const wire = try (ActionButtonsServer{}).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqual(@as(usize, 1 + 132 * 4), wire.len);
        try t.expectEqual(@as(u8, 0), wire[0]);
        try t.expect(std.mem.allEqual(u8, wire[1..], 0));
    }

    {
        const wire = try (InitializeFactionsServer{}).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqual(@as(usize, 4 + 128 * 5), wire.len);
        try t.expectEqual(@as(u32, 128), std.mem.readInt(u32, wire[0..4], .little));
        try t.expect(std.mem.allEqual(u8, wire[4..], 0));
    }

    {
        const wire = try (LoginSetTimeSpeedServer{ .packed_time = 0xDEADBEEF, .speed = 1.0 / 60.0 }).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqual(@as(usize, 12), wire.len);
        try t.expectEqual(@as(u32, 0xDEADBEEF), std.mem.readInt(u32, wire[0..4], .little));
        try t.expectEqual(@as(u32, 0), std.mem.readInt(u32, wire[8..12], .little));
    }

    {
        const wire = try (TimeSyncRequestServer{ .counter = 42 }).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqualSlices(u8, &.{ 42, 0, 0, 0 }, wire);

        const response = try TimeSyncResponseClient.unmarshal(&.{ 42, 0, 0, 0, 9, 8, 7, 6 });
        try t.expectEqual(@as(u32, 42), response.counter);
        try t.expectEqual(@as(u32, 0x06070809), response.client_time_ms);
        try t.expectError(ProtocolError.InvalidMessage, TimeSyncResponseClient.unmarshal(&.{0}));
    }

    {
        const wire = try (InitWorldStatesServer{ .map_id = 0, .zone_id = 12, .area_id = 0 }).marshal(t.allocator);
        defer t.allocator.free(wire);
        try t.expectEqual(@as(usize, 14), wire.len);
        try t.expectEqual(@as(u32, 12), std.mem.readInt(u32, wire[4..8], .little));
        try t.expectEqual(@as(u16, 0), std.mem.readInt(u16, wire[12..14], .little));
    }
}

test "packGameTime matches the AppendPackedTime layout" {
    const t = std.testing;
    // 2024-03-10 12:34 UTC (Sunday): year-2000=24, month-1=2, mday-1=9,
    // wday=0, hour=12, min=34.
    const packed_time = packGameTime(1710074040);
    try t.expectEqual(@as(u32, 24), packed_time >> 24);
    try t.expectEqual(@as(u32, 2), (packed_time >> 20) & 0xF);
    try t.expectEqual(@as(u32, 9), (packed_time >> 14) & 0x3F);
    try t.expectEqual(@as(u32, 0), (packed_time >> 11) & 0x7);
    try t.expectEqual(@as(u32, 12), (packed_time >> 6) & 0x1F);
    try t.expectEqual(@as(u32, 34), packed_time & 0x3F);
}

test "name query response identifies a player" {
    const t = std.testing;
    const query = try NameQueryClient.unmarshal(&.{ 2, 0, 0, 0, 0, 0, 0, 0 });
    try t.expectEqual(ObjectGuid.player(2).valueOf(), query.guid.valueOf());

    const wire = try (NameQueryResponseServer{
        .guid = ObjectGuid.player(2),
        .name = "Human",
        .race = 1,
        .gender = 0,
        .class_id = 1,
...
test "motd carries length-prefixed lines" {
    const t = std.testing;
    const wire = try (MotdServer{ .lines = &.{"hello"} }).marshal(t.allocator);
    defer t.allocator.free(wire);
...
test "destroy object carries the packed guid and death flag" {
    const t = std.testing;

    const wire = try (DestroyObjectServer{ .guid = ObjectGuid.player(0x1234) }).marshal(t.allocator);
    defer t.allocator.free(wire);

    // Packed guid for player(0x1234): mask byte + two nonzero bytes.
    try t.expectEqualSlices(u8, &.{ 0b0000_0011, 0x34, 0x12 }, wire[0..3]);
    // on_death defaults to false.
    try t.expectEqual(@as(u8, 0), wire[3]);
}
